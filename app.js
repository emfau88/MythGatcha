...
const CREATURES = [
  {
    ...
    descriptionDe: 'Urgestalt des Chaos — mächtig, gefürchtet, Hüter uralter Schätze.',
    ...
    loreDe: 'In den tiefsten Höhlen des Schwarzwalds haben Drachen seit Jahrhunderten geschlafen und Horte aus uraltem Gold und vergessener Magie bewacht. Ihre Schuppen schimmern wie Smaragde im Dunkel, ihr Atem kann Stein schmelzen.',
  ...
  },
  {
    ...
    loreDe: 'Unter der Oberfläche von Waldseen und reißenden Flüssen kämmt die Nixe ihr Haar und singt Lieder von verhängnisvoller Schönheit.',
  ...
  },
  {
    ...
    loreDe: 'Nur wenigen Menschen ist es vergönnt, eine Fee zu erblicken. Sie erscheint in sonnendurchfluteten Waldlichtungen, um die Würdigen zu segnen, und bringt Glück und Heilung denen, die reinen Herzens sind.',
  ...
  },
  {
    ...
    loreDe: 'Auf einem schroffen Felsen hoch über dem Rhein sitzt die Lorelei, kämmt ihr goldenes Haar und singt ein Lied von so herzzerreißender Schönheit, dass Schiffer darüber ihr Steuer vergessen.',
  ...
  },
  {
    ...
    loreDe: 'In den nebligen Sümpfen und Moorlandschaften des Schwarzwalds tanzen und flackern geisterhafte Lichter und locken Reisende von sicheren Pfaden in tückische Moore.',
  ...
  },
  {
    ...
    name: 'Eisenhans',
    nameDe: 'Eisenhans',
    description: 'The wild man of the ancient forest — primordial force, silent judge of human courage.',
    descriptionDe: 'Der wilde Mann des Urwalds — urzeitliche Macht und stiller Prüfer menschlichen Muts.',
    lore: 'In a dark pool at the heart of a forgotten forest lives Eisenhans — iron-brown, hair reaching to his knees, dragging hunters and hounds alike into the depths. Once captured and caged by a king, he was freed by a child seeking a golden ball. He carried the boy into the wild and taught him the ways of the ancient world. Though he sends the worthy away, he never abandons them: \'Go out into the world and learn what poverty is — but call my name in the forest, and I will come. My power is great, greater than you think.\'  In the end, it is revealed that Eisenhans is no monster, but an enchanted king — and it was the courage of the innocent that broke his curse.',
    loreDe: 'In einem dunklen Tümpel im Herzen eines vergessenen Waldes lebt Eisenhans — rostbraun am Leib, die Haare bis zu den Knien, Jäger und Hunde gleichermaßen in die Tiefe ziehend. Einst von einem König gefangen und in einen eisernen Käfig gesperrt, wurde er von einem Kind befreit, das seinen goldenen Ball zurückwollte. Eisenhans nahm den Jungen in den Wald und lehrte ihn die Wege der alten Welt. Obwohl er die Würdigen fortschickt, lässt er sie nie fallen: \'Geh hinaus in die Welt und erfahre, wie die Armut tut — doch rufe meinen Namen im Wald, und ich werde kommen. Meine Macht ist groß, größer als du denkst.\' Am Ende offenbart sich: Eisenhans ist kein Ungeheuer, sondern ein verwünschter König — und erst der Mut eines Unschuldigen brach den Fluch.',
    image: 'https://raw.githubusercontent.com/emfau88/MythGatcha/refs/heads/main/creatures/eisenhans.png',
    rarity: 'mythisch',
    region: 'The Forgotten Forest',
    regionDe: 'Der verwunschene Urwald'
  },
  {
    id: 'krampus',
    name: 'Krampus',
...
const TRANSLATIONS = {
  en: {
    ...
    'bestiary.filter.mythisch': 'Mythic',
  ...
  de: {
    ...
    'home.legend': 'Die alten Erzählungen sprechen von Drachen, die Schätze hüten, Zwergen, die tief unter der Erde schürfen, und Geistern, die durch die Nacht streifen...',
    ...
    'bestiary.filter.mythisch': 'Mythisch',
...
const State = {
  ...
  init() {
    ...
    if (saved) {
      ...
      this.drawsSinceNew = data.drawsSinceNew || 0;
    }
  },
  ...
  save() {
    localStorage.setItem('wald-der-alten', JSON.stringify({
      ...
      drawsSinceNew: this.drawsSinceNew
    }));
  },
  ...
  getRandomCreature() {
    const PITY_LIMIT = 15; // after this many draws without new creature -> guaranteed new one
    const unlockedIds = new Set(this.unlockedCreatures.map(c => c.id));
    const allNew = CREATURES.filter(c => !unlockedIds.has(c.id));
    const allKnown = CREATURES.filter(c => unlockedIds.has(c.id));

    // All collected — just draw freely
    if (allNew.length === 0) {
      return this._weightedDraw(CREATURES);
    }

    // Pity trigger: too many draws without a new creature
    if (this.drawsSinceNew >= PITY_LIMIT) {
      return allNew[Math.floor(Math.random() * allNew.length)];
    }

    // Option C: weighted draw — unowned creatures get 3x weight
    const pool = CREATURES.map(c => ({
      creature: c,
      weight: unlockedIds.has(c.id) ? 1 : 3
    }));
    ...
    for (const entry of pool) {
      random -= entry.weight;
      if (random <= 0) return entry.creature;
    }
    return pool[pool.length - 1].creature;
  },

  _weightedDraw(pool) {
    const rarityWeights = { common: 50, rare: 30, epic: 15, legendary: 4, mythisch: 1 };
    const weighted = pool.map(c => ({ creature: c, weight: rarityWeights[c.rarity] || 1 }));
  ...
  async drawCreature() {
    ...
    if (wasNew) {
      ...
      this.drawsSinceNew = 0; // reset pity counter on new discovery
    } else {
      this.drawsSinceNew++;
    }
  ...
  renderHome() {
    return `
      <div class="view home-view">
        ...
        <!-- Zone 1: Title floats in sky/dragon area -->
        <div class="home-title-zone">
          <h1 class="home-title font-fraktur">${this.t('app.title')}</h1>
          <div class="home-divider"></div>
        </div>

        <!-- Zone 2: Content anchored to bottom, giant stays free -->
        <div class="home-content-zone">
          ...
          <div style="margin-top: 16px; font-size: 11px; color: rgba(245, 240, 225, 0.4); text-shadow: 0 0 8px rgba(0,0,0,0.9), 0 2px 4px rgba(0,0,0,1); letter-spacing: 1px;">
            ${CREATURES.length} ${this.t('bestiary.title')}
          </div>
        </div>
    ...
    return `
      <div class="view draw-view">
        ...
        <div class="card-container">
          ...
          ` : showResult ? `
            <div class="card-revealed rarity-${this.lastDrawnCreature.rarity}" onclick="State.viewCreatureDetail()" style="cursor:pointer;">
              <div style="position:absolute;bottom:0;left:0;right:0;z-index:10;display:flex;align-items:center;justify-content:center;gap:6px;padding:8px 0 10px;background:linear-gradient(to top,rgba(0,0,0,0.55) 0%,transparent 100%);font-family:'Cinzel',serif;font-size:11px;color:rgba(245,240,225,0.75);letter-spacing:0.5px;animation:pulse 2s ease-in-out infinite;pointer-events:none;">
      ...
      <div class="view bestiary-view">
        ...
        ${this.showFilters ? `
          <div class="filter-pills">
            ${['all', 'common', 'rare', 'epic', 'legendary', 'mythisch'].map(r => `
  ...
  renderCreatureCard(creature) {
    ...
    const rarityInitial = this.t(`rarity.${creature.rarity}`).charAt(0);
    
    return `
      <div class="creature-card ${isUnlocked ? '' : 'locked'} rarity-border-${creature.rarity}" onclick="State.selectCreatureById('${creature.id}')">
        <img src="${creature.image}" alt="${name}" loading="lazy" onerror="this.style.opacity='0'">
        ...
        <div class="creature-card-info">
          ...
          ` : `
            <div class="creature-card-rarity rarity-${creature.rarity}" style="opacity:0.5;">
              ${rarityInitial}
            </div>
          `}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Set initial lang label to match default language
  const langLabel = document.getElementById('lang-label');
  if (langLabel) langLabel.textContent = State.language.toUpperCase();
...
(function initParticles() {
  ...
  function createParticle(forceBottom) {
    const size = randomBetween(1.5, 3.8);
    return {
      x: randomBetween(0, canvas.width),
      y: forceBottom ? randomBetween(canvas.height * 0.6, canvas.height + 20) : randomBetween(0, canvas.height),
      size,
      // Slight horizontal drift
      vx: randomBetween(-0.12, 0.12),
      // Upward speed
      vy: randomBetween(-0.23, -0.08),
      // Opacity cycling
      opacity: randomBetween(0.1, 0.7),
      opacityTarget: randomBetween(0.4, 0.85),
      opacitySpeed: randomBetween(0.003, 0.009),
      // Color: teal-green spectrum
      hue: randomBetween(155, 185),
      saturation: randomBetween(65, 90),
      lightness: randomBetween(58, 72),
      // Slight wobble
      wobble: randomBetween(0, Math.PI * 2),
      wobbleSpeed: randomBetween(0.005, 0.014),
      wobbleAmp: randomBetween(0.3, 0.9),
    };
  }
  ...
  function drawGlow(p) {
    ...
    ctx.fillStyle = `hsla(${p.hue}, ${p.saturation}%, ${Math.min(p.lightness + 18, 95)}%, ${Math.min(p.opacity + 0.25, 1)})`;
    ctx.fill();
  }
  ...
  function animate() {
    ...
    for (let i = 0; i < particles.length; i++) {
      ...
      p.x += p.vx + Math.sin(p.wobble) * p.wobbleAmp * 0.04;
      p.y += p.vy;
      ...
      if (p.opacity < p.opacityTarget) {
        ...
        p.opacity -= p.opacitySpeed;
        if (p.opacity <= 0.05) {
          p.opacityTarget = randomBetween(0.4, 0.85);
        }
      }

      // Recycle off-screen particles from bottom
      if (p.y < -20 || p.x < -20 || p.x > canvas.width + 20) {
        particles[i] = createParticle(true);
        // Reset x for out-of-bounds horizontal
        if (p.x < -20 || p.x > canvas.width + 20) {
          particles[i].x = randomBetween(0, canvas.width);
        }
        continue;
      }
